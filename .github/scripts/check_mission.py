...
current mission number from .mission-state.json (or a --mission
override), runs that mission's check function against real repo/API
state, and reports pass/fail -- persisting progress to
.mission-state.json and PROGRESS.md on a pass, and always writing a job
summary for immediate feedback in the Actions tab.
"""
...
STATE_PATH = Path(".mission-state.json")
...
def update_progress_md(state: dict) -> None:
...
def bot_commit_progress() -> None:
    subprocess.run(["git", "config", "user.name", "mission-bot"], check=False)
    subprocess.run(
        ["git", "config", "user.email", "mission-bot@users.noreply.github.com"], check=False
    )
    subprocess.run(["git", "add", str(STATE_PATH), str(PROGRESS_PATH)], check=False)
    result = subprocess.run(
        ["git", "commit", "-m", "mission-bot: update progress [skip ci]"], check=False
    )
